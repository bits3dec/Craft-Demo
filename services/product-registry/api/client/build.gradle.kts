...
publishing {
    publications {
        create<MavenPublication>("jar") {
            artifactId = "product-registry-client"
            from(components["java"])
        }
    }
}

val clientPath = "$rootDir/services/product-registry/api/client"
...
val generateClient by tasks.registering(GenerateTask::class) {
    ...
    inputSpec.set("$rootDir/services/product-registry/api/spec.yml")
    outputDir.set(clientPath)

    // config
    generatorName.set("spring")
    library.set("spring-cloud")
    apiPackage.set("demo.craft.product.registry.client.api")
    modelPackage.set("demo.craft.product.registry.client.model")
    configOptions.set(
        mapOf(
